...
from enocean.protocol.constants import PACKET, RORG, PARSE_RESULT, DB0, DB1, DB2, DB3, DB4, DB5, DB6
...
class Packet(object):
    ...
    def _bit_data(self):
        ...
        ####return enocean.utils.to_bitarray(self.data[5:len(self.data) - 5], (len(self.data) - 6) * 8)
    ...
    def parse_msg(buf):
        ...
        Packet.logger.info('Packet.parse_msg')
        ...
        elif packet_type == PACKET.RADIO_ERP2:
            packet = Radio2Packet(packet_type, data, opt_data)
    ...
    def create(packet_type, rorg, rorg_func, rorg_type, direction=None, command=None,
        ...
        self.logger.info('Packet.create')
...
class RadioPacket(Packet):
    ...
    def parse(self):
        ...
        self.logger.info('RadioPacket.parse')
...
class PacketType:
    '''
    Manage table format to analyze ERP2 Header(packet type) and convert to rORG
    '''
    def __init__(self, rorg = 0, org_len = 32, ext_hdr = False, ext_typ = False):
        self.org_len = org_len
        #self.dst_len = 0
        self.ext_hdr = ext_hdr
        self.ext_typ = ext_typ
        #self.erp2hdr = 0
        self.rorg = rorg

    def printall(self):
        print('0x{:02X}'.format(self.rorg), self.org_len, self.ext_hdr, self.ext_typ)


class Radio2Packet(Packet):
    '''
    ERP2: EnOcean Radio Protocol version 2
    '''
    destination = [0xFF, 0xFF, 0xFF, 0xFF]
    dBm = 0
    sender = [0xFF, 0xFF, 0xFF, 0xFF]
    ...
    ptype = {
        0x20: PacketType(0xF6, 32), #RPS
        0x21: PacketType(0xD5, 32), #1BS
        0x22: PacketType(0xA5, 32), #4BS
        0x23: PacketType(0xD0, 32), #SmAck
        0x24: PacketType(0xD2, 32), #VLD
        0x25: PacketType(0xD4, 32), #UTE
        0x26: PacketType(0xD1, 32), #MSC

        0x60: PacketType(0xF6, 48), #RPS Teach-in 48Bit
        0x61: PacketType(0xD5, 48), #1BS Teach-in 48Bit
        0x62: PacketType(0xA5, 48), #4BS Teach-in 48Bit
        0x65: PacketType(0xD4, 48), #UTE Teach-in 48Bit
        0x66: PacketType(0xD1, 48), #MSC Teach-in 48Bit
       }

    def __str__(self):
        packet_str = super(Radio2Packet, self).__str__()
        return '%s->%s (%d dBm): %s' % (self.sender_hex, self.destination_hex, self.dBm, packet_str)

    @staticmethod
    def create(rorg, rorg_func, rorg_type, direction=None, command=None,
               destination=None, sender=None, learn=False, **kwargs):
        return Packet.create(PACKET.RADIO_ERP2, rorg, rorg_func, rorg_type, direction, command, destination, sender, learn, **kwargs)
...
#    def _bit_data(self):
#        ########## ERP2 special
#        return enocean.utils.to_bitarray(self.data[self.data_offset:len(self.data) - self.data_offset], (len(self.data) - self.data_offset - 1) * 8)

    def parse(self):
        ##self.destination = self.optional[1:5] 
        ##self.dBm = -self.optional[5]
        self.dBm = -self.optional[1]
        ##self.sender = self.data[-5:-1]
        try:
            sender_len = self.ptype[self.data[0]].org_len
        except:
            sender_len = 32

        self.sender = self.data[3:7] if sender_len == 48 else self.data[1:5]
        self.data_offset = sender_len // 8 + 1
        ...
        # self.rorg = self.data[0]
        try:
            self.rorg = self.ptype[self.data[0]].rorg
        except:
            self.rorg = 0

        self.logger.info('Radio2Packet.parse rorg:0x{:02X}'.format(self.rorg))

        # parse learn bit and FUNC/TYPE, if applicable
        if self.rorg == RORG.RPS:
            self.learn = True
            self.logger.debug('learn received RPS')
        elif self.rorg == RORG.BS1:
            ##self.learn = not self._bit_data[DB0.BIT_3]
            self.learn = not (self.data[self.data_offset] & 0x08)
            if self.learn:
                self.logger.debug('learn received 1BS')
        elif self.rorg == RORG.BS4:
            ##self.learn = not self._bit_data[DB0.BIT_3]
            self.learn = not (self.data[self.data_offset + 3] & 0x08)
            if self.learn:
                pass

            #self.logger.debug('4BSdata.parse: ofs=%d len=%d [%02X %02X]' % (self.data_offset, len(self.data) - 5, self.data[self.data_offset], self.data[self.data_offset + 1]))
            #self.logger.debug('4BSdata.parse data[] (%s)' % ([self.data[self.data_offset:len(self.data) - 5]]))
            #self.logger.debug('4BSdata.parse DB3:DB0, %02X %02X %02X %02X %02X %02X' % (
            #    enocean.utils.from_bitarray(self._bit_data[DB5.BIT_7:DB5.BIT_0]),
            #    enocean.utils.from_bitarray(self._bit_data[DB4.BIT_7:DB4.BIT_0]),
            #    enocean.utils.from_bitarray(self._bit_data[DB3.BIT_7:DB3.BIT_0]),
            #    enocean.utils.from_bitarray(self._bit_data[DB2.BIT_7:DB2.BIT_0]),
            #    enocean.utils.from_bitarray(self._bit_data[DB1.BIT_7:DB1.BIT_0]),
            #    enocean.utils.from_bitarray(self._bit_data[DB0.BIT_7:DB0.BIT_0]),))
            ...
            if self.learn:
                #self.contains_eep = self._bit_data[DB0.BIT_7]
                    ...
                    #self.rorg_type = enocean.utils.from_bitarray(self._bit_data[DB3.BIT_1:DB2.BIT_2])
                    #self.rorg_manufacturer = enocean.utils.from_bitarray(self._bit_data[DB2.BIT_2:DB0.BIT_7])
                self.rorg_func = (self.data[self.data_offset + 0] >> 2) & 0x3F
                self.rorg_type = ((self.data[self.data_offset + 0] & 0x03) << 5) | ((self.data[self.data_offset + 1] >> 3) & 0x1F)
                self.rorg_manufacturer = ((self.data[self.data_offset + 1] & 0x07) << 8) | self.data[self.data_offset + 2]
                self.logger.debug('learn received 4BS, EEP %02X-%02X-%02X ManID: 0x%03X' % (self.rorg, self.rorg_func, self.rorg_type, self.rorg_manufacturer))

        elif self.rorg == RORG.UTE:
            #return UTETeachInPacket(packet_type, data, opt_data).parse()
            self.channel = self.data[self.data_offset + 1]
            self.rorg_type = self.data[self.data_offset + 4]
            self.rorg_func = self.data[self.data_offset + 5]
            self.rorg_of_eep = self.data[self.data_offset + 6]
            self.rorg_manufacturer = ((self.data[self.data_offset + 3] & 0x07) << 8) | self.data[self.data_offset + 2]
            self.logger.debug('UTE received, EEP:%02X-%02X-%02X ManID: 0x%03X' % (self.rorg_of_eep, self.rorg_func, self.rorg_type, self.rorg_manufacturer))

        else:
            self.learn = False

        # If not learn, process some data
        if not self.learn:
            self.logger.debug('not learn received, RORG: 0x%02X' % self.rorg)
            # continue to application level

        return super(Radio2Packet, self).parse()
